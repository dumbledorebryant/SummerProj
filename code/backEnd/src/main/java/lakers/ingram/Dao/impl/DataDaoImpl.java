...
public class DataDaoImpl implements DataDao {
    public List<DataEntity> getInitDataByDate(Timestamp date){
        ...
        Query query =session.createQuery("select data from DataEntity data " +
                ...
                .setParameter("dt",date).setParameter("ds",new Timestamp(date.getTime()-1800000));
    ...
    public DataEntity getCurrentData(){
        Session session=HibernateUtil.getSession();
        session.beginTransaction();
        Query query = session.createQuery("select data from DataEntity data order by date desc");
        @SuppressWarnings("unchecked")
    ...
    public List<DataEntity> getHistoryDataByDate(Timestamp date){
        ...
        Query query =session.createQuery("select data from DataEntity data " +
                "where data.date <= :dt and data.date >= :ds order by date asc")
                .setParameter("dt",new Timestamp(time+1800000)).setParameter("ds",new Timestamp(time-1800000));
        @SuppressWarnings("unchecked")
        List<DataEntity> ds= query.list();
        session.getTransaction().commit();
        return ds;
    ...
    public DataEntity getCurrentHistoryDataByDate(Timestamp date){
        ...
        time=time-86400000;
        Query query =session.createQuery("select data from DataEntity data " +
                "where data.date <= :dt and data.date >= :ds order by date desc")
